...

{
  ...
  # the guest:
  #   * Older + v6 series: Hyper-V SCSI (root appears as /dev/sda).
  #   * v7 series (Dasv7/Easv7): NVMe-only (root appears as /dev/nvme0n1).
  # To make a single image bootable on both, the initramfs must contain the
  # drivers for both controllers; otherwise stage-1 cannot find the root
  ...
  boot.initrd.availableKernelModules = [
    ...
    "nvme"
    "nvme_core"
    # Hyper-V SCSI / VMBus / netvsc (v6 and earlier, default path on v6)
    "hv_storvsc"
    "hv_vmbus"
    "hv_netvsc"
  ];
